...
function fixDescription(desc: string): string {
  ...
  // Fix Étouffée capitalization
  // "Shrimp étouffée" when it's the dish → "Shrimp Étouffée"
  fixed = fixed.replace(/\bShrimp étouffée\b/g, 'Shrimp Étouffée');
  fixed = fixed.replace(/\bShrimp Etouffee\b/gi, 'Shrimp Étouffée');

  // But standalone étouffée stays lowercase unless at sentence start
  ...
  // Lowercase "bed of"
  fixed = fixed.replace(/\bBed of\b/g, 'bed of');

  // Lowercase "sides" when referring to side dishes
  fixed = fixed.replace(/\bSides\b/g, 'sides');

  // Lowercase "garlic bread"
  fixed = fixed.replace(/\bGarlic Bread\b/g, 'garlic bread');

  // Lowercase descriptive sauces
  fixed = fixed.replace(/\bCreamy Cajun Shrimp Sauce\b/g, 'creamy cajun shrimp sauce');

  // Lowercase produce/vegetables
  fixed = fixed.replace(/\bGreen Bell Peppers\b/g, 'green bell peppers');
