...
function fixDescription(desc: string): string {
  ...
  // Lowercase "filet"
  fixed = fixed.replace(/\bFilet\b/g, 'filet');
  ...
  // Fix cooking methods - should be lowercase everywhere, including when standalone
  fixed = fixed.replace(/\b(Fried|Grilled|Blackened|Boiled)\b/g, (match) => {
